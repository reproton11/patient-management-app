// patient-management-app/frontend/src/components/ui/selectStyles.js
// Tema react-select sesuai design system Klinik AZ, dipakai SelectInput
// (opsi statis) dan AsyncSelectInput (opsi dari API).
// Menu di-portal ke document.body (z-65) agar tidak pernah tertutup
// kartu kaca lain, tabel, maupun modal (z-50).
export const buildStyles = (error) => ({
  control: (base, state) => ({
    ...base,
    minHeight: 42,
    borderRadius: 8,
  ...
  option: (base, state) => ({
    ...
    borderRadius: 8,
    padding: "8px 10px",
    color: state.isSelected || state.isFocused ? "#155e75" : "#374151",
    backgroundColor:
